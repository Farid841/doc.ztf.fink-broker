...
In general, you can follow what is done for the [continuous integration](https://github.com/astrolabsoftware/fink-broker/blob/master/.github/workflows/sentinel-template.yml). Once the image `gitlab-registry.in2p3.fr/astrolabsoftware/fink/fink-deps-sentinel-ztf:latest` (GitLab registry) is pulled, fire a container and set the `PYTHONPATH` and `PATH` to use the tools:
